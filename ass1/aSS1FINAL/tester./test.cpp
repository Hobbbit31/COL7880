...
bool verifyTotalAmount(const DataSet& data) {
    ...
    int64_t actual = totalAmountTraded(data.orders);
...
void runPerformanceTest(size_t n, uint32_t s, int32_t f) {
    ...
    bench("totalAmountTraded", [&](){ totalAmountTraded(orders); });
    bench("printOrderStats", [&](){ printOrderStats(orders); });
    bench("updateDisplay", [&](){ updateDisplay(orders, f); });
}
