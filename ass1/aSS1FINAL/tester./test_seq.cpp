...
bool verifyTotalAmount(const DataSet& data) {
    ...
    int64_t actual = totalAmountTraded_seq(data.orders);
...
bool verifyStatsFile(const DataSet& data) {
    ...
    printOrderStats_seq(data.orders); 
...
void runPerformanceTest(size_t n, uint32_t s, int32_t f) {
    ...
    bench("totalAmountTraded", [&](){ totalAmountTraded_seq(orders); });
    bench("printOrderStats", [&](){ printOrderStats_seq(orders); });
    bench("updateDisplay", [&](){ updateDisplay_seq(orders, f); });
}
